...
$sql = "INSERT INTO pessoas (nome, cnpj, razao_social, nome_fantasia, endereco, contato, tipo_pessoa, cargo) VALUES (?, ?, ?, ?, ?, ?, 2, 5)";
...
$stmt->bind_param("ssssss", $nome, $cnpj, $razao_social, $nome_fantasia, $endereco, $contato);

$stmt_fornecedor->bind_param("s",$cnpj);

if($stmt->execute() and $stmt_fornecedor->execute()){
    $resposta = "Fornecedor cadastrado com sucesso!";
    header("Location: ../../admin/fornecedores.php?resposta=$resposta");
    die();
} else {
